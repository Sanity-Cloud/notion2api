...
MAX_GUARDED_STREAM_BUFFER_CHUNKS = 10_000
...
def _build_stream_error_event(response_id: str, model: str, outcome: Any) -> str:
    """Emit a terminal-safe SSE error receipt after an upstream stream failure."""
    payload = {
        "id": response_id,
        "object": "error",
        "model": model,
...
def _guard_stream_until_integrity(
    ...
    *,
    ...
    """Buffer provider SSE until a valid terminal receipt is observed.

    The buffered clean-stream path remains byte-for-byte compatible.  A provider
    exception after HTTP headers have been sent is converted into an explicit SSE
    error plus an error terminal, instead of escaping as a zero-byte success.
    """
    buffered: list[str] = []
    ...
    try:
        for raw_chunk in source:
            ...
            if (
                not forced_limit
                and total_chars <= MAX_GUARDED_STREAM_BUFFER_CHARS
                and total_chunks <= MAX_GUARDED_STREAM_BUFFER_CHUNKS
            ):
                buffered.append(chunk)
            else:
                forced_limit = True
                buffered.clear()
                # Do not drain an unbounded upstream after a fail-closed breach.
                break
            ...
            if event_type == "model_metadata" and not forced_limit:
                metadata_events.append(chunk)
            if event_type == "output_hygiene" and not forced_limit:
    ...
    if forced_limit:
        ...
        if total_chunks > MAX_GUARDED_STREAM_BUFFER_CHUNKS:
            limit_reasons.append("guarded_stream_buffer_chunk_limit_exceeded")
        hygiene = {
            "output_integrity": assess_output_integrity(
                "", additional_reasons=tuple(limit_reasons)
            )
        }
        yield from metadata_events
        yield _build_hygiene_metadata_event(hygiene)
        yield _build_stream_chunk(response_id, model, finish_reason="content_filter")
        # Guarded content-filter outcomes are non-successes; do not emit the
        # success-only sentinel because bundled consumers do not classify it.
        return
    ...
    if not outcome.ok:
        ...
        yield _build_stream_error_event(response_id, model, outcome)
        yield _build_stream_chunk(response_id, model, finish_reason="error")
        # Do not emit the successful-stream sentinel after a failure: both the
        # bundled frontend and MCP client treat [DONE] as an unqualified close.
        return
