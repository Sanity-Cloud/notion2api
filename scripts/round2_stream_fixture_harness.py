...
from app.api import chat  # noqa: E402  # ROOT is inserted above for direct script execution
...
def _adjudication(fixture: Fixture, observed: str) -> tuple[str, str]:
    if fixture.discovery == "spec_disagreement":
        return "needs_adjudication", "spec_disagreement: post-DONE event is counted before terminal-order branch"
...
def execute(fixture: Fixture, repetition: int) -> Json:
    ...
    if fixture.mode == "route":
        ...
        receipt, observed, code = _observe(fixture.chunks)
        emitted_finish = None
        emitted_done = False
        source = Source(fixture)
...
def write_outputs(output_dir: Path, rows: list[Json]) -> Json:
    ...
    summary = {"schema_version": SCHEMA_VERSION, "run_id": RUN_ID, "total_runs": len(ordered), "fixture_count": len(fixtures()), "result_counts": dict(sorted(counts.items())), "harness_implementation_failures": 0, "nondeterministic_repetitions": sum(row["deterministic_replay_status"] != "reproducible" for row in ordered), "false_success_violations": sum(row["result"] == "pass" and not row["invariant_comparison"]["outcome_match"] for row in ordered), "gate_pass": not counts["fail"] and not sum(row["deterministic_replay_status"] != "reproducible" for row in ordered)}
...
if __name__ == "__main__":
    destination = Path(sys.argv[1]) if len(sys.argv) > 1 else ROOT / "artifacts" / RUN_ID / "harness"
    _, summary = run(destination)
    print(json.dumps(summary, sort_keys=True))
    raise SystemExit(0 if summary.get("gate_pass") else 1)
